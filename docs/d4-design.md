...
> **范围**：license-client v1.0.0-alpha.2 / beta.1 的实现规划
> **关联**：[License-Mgr#2](https://github.com/Clouditera/license-client/issues/2)，requirements.md §F2 / F5 / F8 / M1 / N4
...
|---|---|---|---|
| **Q-1** PROD_TOKEN_KEY 嵌入 | (a) tsup `define` 替换 PLACEHOLDER | **撤回。改为字节嵌入真实 PEM** | CLI 现行做法是直接嵌入 PEM（不再用 PLACEHOLDER）。license-client 已有的 PROD_KEY 也是直接嵌入。三方对齐 + R1 字节等价要求。tsup `define` 会让 license-client 跟 CLI 在 build 工艺分叉，无对称收益 |
...
注：env 名同步发现 license-client 仓库内 `CORTEXDEV_*` 命名（如 `CORTEXDEV_LICENSE_API_URL` / `CORTEXDEV_PUBLIC_KEY`）也面临同样问题。本设计**仅**修正与 D4 直接相关的 token env 名；其它 env 命名一致化属于独立 scope（参 §7 Q-5）。
...
license-client alpha.1 是 DevAgent-App 的副本，**不是** App + CLI 的并集。CLI legacy 已在生产实现的 D4 (`online_check_token`) 与 hostname allowlist 完全缺失，使 §一.2 "single source of truth" 的核心目标失败、§N5 R1 "字节级行为等价" 的硬约束被违反、Phase 3 CLI 接入会直接触发功能 + 安全降级。

### 1.2 R1 违反清单（按 CLI legacy 实现编排）

| 维度 | CLI legacy | license-client alpha.1 | 缺口性质 |
|---|---|---|---|
...
- 在 license-client 内补齐上表所有缺口，**行为字节级对齐 CLI legacy**
...
- ❌ 重新设计 LicenseService 状态机（CLI gate.js 与 LicenseService 的语义映射靠 adapter 层做，不改 license-client 状态机）
...
**理由**：保持 validator 是 pure 7 步（与 §F2 现有契约对齐，CLI 接入 adapter 通过 `service.checkOfflineGrace()` 拿决策结果，不污染 validator）。CLI gate.js 的 `checkOfflineGrace` 在 license-client 里就是 `LicenseService.checkOfflineGrace()`，签名同构，迁移成本最低。
...
- 字节复用 CLI 现有脚本，仅必要的路径调整（license-client 仓库内 `src/token-key.ts` vs CLI 的 `packages/core/src/license/token-key.js`）
...
license-client alpha.1 现在是 `https://license.clouditera.online`（无 path）。

R1 要求字节对齐 → alpha.2 改成 `https://license.clouditera.online/api/v1`。

后果：现有 license-client alpha.1 调用方（无）会感知到 base URL 变化，但因为 alpha.1 还没有任何下游接入，无回归风险。
...
1. **同一 mock /activate response** 喂给 license-client core 与 CLI legacy（通过 spawn child process 调 CLI），断言两边：
...
2. **同一签名 D4 token 喂给 license-client 与 CLI legacy verifyOnlineCheckToken**，断言 verdict 完全相同。
...
1. license-client ship v1.0.0-alpha.2
2. CLI 仓库新分支 `feat/license-client-adapter`，依赖切到 `git+ssh://...#v1.0.0-alpha.2`
...
**v0.3 修正决策**：**字节嵌入真实 PEM**（与 CLI legacy / license-client 现有 PROD_KEY 三方对齐）。

**撤回原因**（详见 §0 变更摘要）：
- CLI 现行做法是源码直接嵌入真实 PEM（不再用 PLACEHOLDER）
- license-client 已有的 `crypto.ts: PROD_KEY` 也是直接嵌入真实 PEM
- 用 tsup `define` 会让 license-client 跟 CLI 在 build 工艺上分叉——无对称收益，破坏 R1 字节等价
- 公钥不是机密；进 git 历史完全合规
...
- license-client 新 adapter：同样行为，但调用 `service.checkOfflineGrace()` 拿 result
...
### Q-5：license-client 内部 `CORTEXDEV_*` env 命名是否一致化为 `DEVAGENT_*` — **本设计不处理**

**背景**：CLI 已经完成 `cortexdev → devagent` 品牌重命名，env 名变成 `DEVAGENT_LICENSE_SERVER` / `DEVAGENT_TOKEN_PUBLIC_KEY` / `DEVAGENT_PRO_BUILD` 等。license-client 仓内 alpha.1 还在用 `CORTEXDEV_LICENSE_API_URL` / `CORTEXDEV_PUBLIC_KEY` / `CORTEXDEV_CONFIG_DIR`。
...
**Follow-up**：独立开 issue 跟踪 license-client 全仓 env 命名一致化（建议在 alpha.2 ship 之后做，避免 D4 工作被拖入命名争论）。本设计落地时只动 token env 名，其它保持现状。
...
license-client v1.0.0-alpha.2 发布前必须满足：
...
- License-Mgr#2: license-client missing D4 + hostname allowlist (CLI legacy already ships them)
