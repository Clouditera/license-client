# Phase 3 — DevAgent-CLI adopts `@clouditera/license-client@1.0.0-alpha.2`

> **状态**：草案 v0.1
> **日期**：2026-06-17
> **作者**：lc-kendo（与 AI 协作起草）
> **范围**：CLI side of the Phase 3 migration described in `docs/requirements.md` §1.4
> **依赖**：`@clouditera/license-client@1.0.0-alpha.2` (tag `v1.0.0-alpha.2`, commit `cfaaa05`)
> **关联**：[License-Mgr#2 closed](https://github.com/Clouditera/license-client/issues/2)，docs/d4-design.md

---

## 0. 目的

DevAgent-CLI 现状有一份完整的内嵌 license 实现（`packages/core/src/license/` + `packages/devagent-pro/src/license/`）。Phase 3 的目的是让 CLI **跑** `@clouditera/license-client` 作为统一信任源，**不丢任何功能、不改任何客户行为**，再按 §1.4 Phase 4 灰度全量、Phase 5 删旧码。
...
{
  "dependencies": {
    "@clouditera/license-client": "git+ssh://git@github.com/Clouditera/license-client.git#v1.0.0-alpha.2"
  }
}
```
...
license-client 用 module-level setter 模式注入宿主信息（不是构造参数）。CLI bootstrap（`packages/devagent-pro/src/cli.js`）在最早的 license 调用之前必须跑一次：
...
// Route license-client logger through CLI's existing logger
...
- `offlineGraceDays: 60` — preserves CLI 60-day legacy grace (license-client default is 14, which would be a downgrade)
...
## 3. API 映射表（CLI legacy → license-client）

| CLI legacy 调用点 | 现状 API | 替换为 license-client API | 字节等价? |
|---|---|---|---|
...
| `packages/core/src/license/validator.js: validateLicense(...)` | 5-step pipeline | `import { validateLicense } from '@clouditera/license-client'` — runs 7-step pipeline (incl. clock A/B anti-rollback) | ⚠️ stricter (see §3.1) |
| `packages/core/src/license/crypto.js: verifySignature` | 直接调用 | `import { verifySignature } from '@clouditera/license-client'` | ✅ canonicalize byte-identical |
...
| `packages/devagent-pro/src/license/gate.js: updateOnlineCheckTimestamp(configDir)` | bumps last_online_check preserving signed_token | currently no direct API — gate semantics fold into `_handleRefreshSuccess` | ⚠️ 需要 adapter shim 或 license-client 加 API |
| `packages/devagent-pro/src/license/online-check.js: verifyOnlineCheckToken(...)` | local verifier | `import { verifyOnlineCheckToken } from '@clouditera/license-client'` | ✅ |
| `packages/core/src/license/token-key.js: EMBEDDED_TOKEN_PUBLIC_KEY` | local key | `import { EMBEDDED_TOKEN_PUBLIC_KEY } from '@clouditera/license-client'` | ✅ same PEM bytes |
| `packages/core/src/license/server-url.js: resolveLicenseServerURL` | env + allowlist | license-client applies allowlist internally; CLI no longer calls this directly | ✅ |

### 3.1 Validator 步数差异

license-client `validateLicense` 跑 7 步（多 `issued_at - 60s` 防回拨 + `lastVerifiedAt - 60s` 防回拨）；CLI legacy 跑 5 步。
...
**处理**：这是 license-client 的预期行为（防 grace 时间被人手动延长）。Phase 4 灰度对照监控"拒签率"是否突涨——若涨，定位是不是时钟回拨场景占多大比例，再决定是否 expose 一个 `skipClockChecks: boolean` 开关。

### 3.2 Refresh schedule 差异

| | CLI legacy | license-client alpha.2 |
|---|---|---|
...
**处理选项**：
- (A) license-client 加 `HostEnvironment.refreshIntervalMs?: number` 注入点，CLI 传 `3 * 24 * 60 * 60 * 1000`。**已落地于 v1.0.0-alpha.3**——同时加了 `refreshStartupBudgetMs?: number`（CLI 传 5000，匹配 `STARTUP_BUDGET_MS`）。
...
CLI gate.js 在 license 启动时（且 license valid）调 `updateOnlineCheckTimestamp(configDir)`——**保留**已有 `signed_token` + `server_time` 同时刷新 `last_online_check`。license-client 没有直接对应的方法。

**两种解决**：
- (A) license-client 加 `LicenseService.bumpLastOnlineCheck()`，封装"读 + 重写保留可选字段"
- (B) CLI adapter 直接 `import { readOnlineCheck, writeOnlineCheck } from '@clouditera/license-client'`，自己写 1 行 helper
...
3. **Phase 5 删除 legacy** — 移除 `packages/core/src/license/` + `packages/devagent-pro/src/license/{gate,activate,refresh,validator,...}.js`，保留 adapter shim 作为 thin layer 或直接 inline 调 license-client。同时移除 `CORTEXDEV_LICENSE_IMPL` env。
...
| # | 项 | 解决方 | 风险 | 状态 |
|---|---|---|---|---|
| B1 | `refreshIntervalMs` / `refreshStartupBudgetMs` HostEnvironment 注入点（§3.2） | license-client v1.0.0-alpha.3 | 中 — 不解决 CLI 流量翻 3x | ✅ **解决于 [v1.0.0-alpha.3](https://github.com/Clouditera/license-client/releases/tag/v1.0.0-alpha.3) / License-Mgr#3** |
...
建议：
- B1 立刻在 license-client 开 issue + 1-2 天内 ship alpha.3
- B2 / B3 / B4 可以并行做
...
- [ ] 文档更新：CLI README + CHANGELOG 标注 license-client 依赖 + 灰度方式
...
- License-Client v1.0.0-alpha.2 tag: `cfaaa05`
